			...
			function banker()
			{
						...
						for(i=0;i<pro;i++)
						{
							...
							for(j=0;j<res;j++)
							{
								...
								cell.innerHTML = needy[i][j];
								console.log(needy[i][j]);
							}
						}
                        //TABLE for ALLOC
							...
							for (i = 0; i < pro; i++)
							{
								...
								if(ex[i] == 0)
								{
									...
									if(flag)
									{
                                        ...
                                        prints.appendChild(ul1);
                                        var li = document.createElement("li");
                                        li.appendChild(document.createTextNode("Need: " + ned[i]));
                                        ul2.append(li);
                                        ul1.append(ul2);
                                        prints.appendChild(ul1);
                                        var li = document.createElement("li");
							...
							if(exec == pro)
								{
									...
									for(var k = 0; k < safe_state.length; k++){
										li.appendChild(document.createTextNode("P" + safe_state[k] + " "));
									}
                                    prints.appendChild(li);
									break;
								}
